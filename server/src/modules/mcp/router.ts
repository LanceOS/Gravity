...
export class McpRouterFactory {
    ...
    create() {
        ...
        const createLimiter = env.redisEnabled
        ...
        const workspaceTransportLimiter = createLimiter({
            ...
            keyFn: async (req) => {
                const headerWorkspaceId = (
                    req.header("x-workspace-id") || req.header("X-Workspace-Id")
                )?.trim();
                const bodyWorkspaceId =
                    typeof req.body?.params?.workspaceId === "string" &&
                    req.body.params.workspaceId.trim().length > 0
        ...
        const transportIpLimiter = createLimiter({
            windowMs: 60_000,
            max: 300,
            ...
            async (req: Request, res: Response) => {
                try {
                    ...
                    if (
                        ...
                        const possibleToken = rawAuthHeader
                            .slice("bearer ".length)
                            .trim();
                    ...
                    if (actorUserId) {
                        const hasAccess = await isWorkspaceMember(
                            workspaceId,
                            actorUserId,
                        );
                        if (!hasAccess) {
                        ...
                        if (bearerMatch) {
                            ...
                            if (token) {
                                ...
                                const wsKey = workspaceId
                                    ...
                                    : null;
                                if (
                                    (await isBlocked(ipKey)) ||
                                    (wsKey && (await isBlocked(wsKey)))
                                ) {
                                    res.status(429).json({
                                        error: "Too many authentication attempts; try later.",
                                    });
                                    return;
                                }
                                try {
                                    const { verifyAndConsumeToken } =
                                        await import("./connection.js");
                                    const sourceIp = getRequestSourceIp(req);
                                    const tokenRow =
                                        await verifyAndConsumeToken(
                                            token,
                                            workspaceId,
                                            { sourceIp },
                                        );
                                    ...
                                    tokenScopes = Array.isArray(tokenRow.scopes)
                                        ? tokenRow.scopes
                                        : [];
